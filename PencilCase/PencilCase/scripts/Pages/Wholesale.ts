...
export class Wholesale extends PageBase {
    ...
    private wholesaleOrders = ko.observableArray<Order>([]);
    ...
    private addOrder = () => {
        ...
        let isNew = true;
        for (let i = 0; i < this.wholesaleOrders().length; i++) {
            if (this.wholesaleOrders()[i].product().Id === this.selectedProduct().Id) {
                // order.total is a ko.computed observable, so, when quantity changed, the total will be updated automatically
                this.wholesaleOrders()[i].quantity(this.wholesaleOrders()[i].quantity() + this.selectedProductQuantity());
                this.totalPrice(this.totalPrice() + this.selectedProductQuantity() * this.selectedProduct().WholesalePrice)
                isNew = false;
                break;
            }
        }
        ...
        if (isNew) {
            let order = new Order(Utils.guid(), this.batchId, this.selectedProduct(), OrderTypes.Import, this.selectedProductQuantity(), this.selectedProduct().WholesalePrice);
            this.wholesaleOrders.push(order);
    ...
    private deleteOrder = (order: Order) => {
        this.wholesaleOrders.remove(order);
    ...
    private save = () => {
        for (let i = 0; i < this.wholesaleOrders().length; i++) {
            let order = this.wholesaleOrders()[i];
